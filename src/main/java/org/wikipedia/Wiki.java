...
public class Wiki implements Serializable
{
    ...
    protected static final String version = "0.31";

    // the domain of the wiki
    protected String domain;
    protected String query, base, apiUrl;
    protected String scriptPath = "/w";
    protected boolean wgCapitalLinks = true;
    protected String timezone = "UTC";

    // user management
    protected Map<String, String> cookies = new HashMap<>(12);
    protected User user;
    protected int statuscounter = 0;

    // various caches
    protected transient LinkedHashMap<String, Integer> namespaces = null;
    protected transient List<String> watchlist = null;

    // preferences
    protected int max = 500;
    protected int slowmax = 50;
    protected int throttle = 10000; // throttle
    protected int maxlag = 5;
    protected int assertion = ASSERT_NONE; // assertion mode
    protected transient int statusinterval = 100; // status check
    protected String useragent = "Wiki.java/" + version + " (https://github.com/MER-C/wiki-java/)";
    protected boolean zipped = true;
    protected boolean markminor = false, markbot = false;
    protected boolean resolveredirect = false;
    ...
    protected long lastThrottleActionTime = 0;
    // Store time when next login attempt can be executed
    protected long nextLoginTime = -1;

    // retry flag
    protected boolean retry = true;

    // serial version
    protected static final long serialVersionUID = -8745212681497643456L;
    ...
    protected static final int LOG2_CHUNK_SIZE = 22;
     ...
     *  configuration of the wiki. One example use is server-side org.wikipedia.cache
     ...
     *  namespace org.wikipedia.cache
     *  @see #namespaceIdentifier(int)
     *  @since 0.03
     */
    public int namespace(String title) throws IOException
    {
        // org.wikipedia.cache this, as it will be called often
        if (namespaces == null)
        ...
        // look up the namespace of the page in the namespace org.wikipedia.cache
     ...
     *  @throws IOException if the namespace org.wikipedia.cache has not been populated, and
    ...
    /**
     *  Populates the namespace org.wikipedia.cache.
     *  @throws IOException if a network error occurs.
     *  @since 0.25
     */
    ...
    /**
     *  Purges the server-side org.wikipedia.cache for various pages.
        ...
        Revision[] revisions) throws IOException, LoginException
    {
        ...
        if (Objects.equals(hidecontent, Boolean.TRUE))
            out.append("content%7C");
        else if (Objects.equals(hidecontent, Boolean.FALSE))
            temp.append("content%7C");
        if (Objects.equals(hideuser, Boolean.TRUE))
            out.append("user%7C");
        else if (Objects.equals(hideuser, Boolean.FALSE))
            temp.append("user%7C");
        if (Objects.equals(hidereason, Boolean.TRUE))
            out.append("comment");
        else if (Objects.equals(hidereason, Boolean.FALSE))
            temp.append("comment");
    ...
    protected Revision parseRevision(String xml, String title)
    {
        ...
        if (xml.contains("sha1=\""))
            revision.setSha1((parseAttribute(xml, "sha1", 0)));
        return revision;
    }
    ...
    {
        return contribs(user, "", null, null, false, ns);
    }
    ...
    public Revision[] rangeContribs(String range) throws IOException
    {
        ...
        {
            case 8:
                return contribs("", numbers[0] + ".", null, null, false);
            case 16:
                return contribs("", numbers[0] + "." + numbers[1] + ".", null, null, false);
            case 24:
                return contribs("", numbers[0] + "." + numbers[1] + "." + numbers[2] + ".", null, null, false);
            case 32: // not that people are silly enough to do this...
                return contribs(range.substring(0, range.length() - 3), "", null, null, false);
            default:
     ...
     *  @since 0.17
     */
    public Revision[] contribs(String user, String prefix, Calendar end, Calendar start, boolean justcheck, int... ns) throws IOException
    {
        // prepare the url
        StringBuilder temp = new StringBuilder(query);
        temp.append("list=usercontribs&uclimit=");
        temp.append(justcheck ? 1 : max);
        temp.append("&ucprop=title%7Ctimestamp%7Cflags%7Ccomment%7Cids%7Csize%7Csizediff&");
        ...
        while (uccontinue != null && !justcheck);
    ...
    {
        // create the watchlist org.wikipedia.cache
     ...
     *  Fetches the list of titles on the currently logged in user's watchlist.
     *  Equivalent to [[Special:Watchlist/raw]].
     *  @param cache whether we should use the watchlist org.wikipedia.cache
     *  (no online activity, if the org.wikipedia.cache exists)
     *  @return the contents of the watchlist
     *  @throws IOException if a network error occurs
     *  @throws CredentialNotFoundException if not logged in
     *  @since 0.18
    ...
    /**
     *  Determines whether a page is watched. (Uses a org.wikipedia.cache).
    ...
    {
        // populate the watchlist org.wikipedia.cache
        if (watchlist == null)
            getRawWatchlist();
...
//    }
        ...
        int maximum, Boolean redirects) throws IOException
    {
        ...
        if (Objects.equals(redirects, Boolean.TRUE))
            url.append("&apfilterredir=redirects");
        else if (Objects.equals(redirects, Boolean.FALSE))
    ...
    public class User implements Cloneable, Serializable
    {
        ...
        protected String[] rights = null; // org.wikipedia.cache
        protected String[] groups = null; // org.wikipedia.cache
        ...
        public Map<String, Object> getUserInfo() throws IOException
        {
            String url = query + "list=users&usprop=editcount%7Cgroups%7Crights%7Cemailable%7Cblockinfo%7Cgender%7Cregistration&ususers="
                + encode(username, false);
            String info = fetch(url, "getUserInfo");
            Map<String, Object> ret = new HashMap<>(10);

            ret.put("userid", parseAttribute(info, "userid", 0));
            ...
            ret.put("gender", Gender.valueOf(parseAttribute(info, "gender", 0)));
                ...
                log(Level.SEVERE, "getUserInfo", "Exception: " + ex.getMessage() + "\n" + url);
            }
         ...
         *  Uses the rights org.wikipedia.cache. Read [[Special:Listgrouprights]] before using
         *  this!
         ...
         *  the groups org.wikipedia.cache.
    ...
    {
        protected boolean minor, bot, rvnew;
        protected String summary;
        protected long revid, rcid = -1;
        protected long previous = 0, next = 0;
        protected Calendar timestamp;
        protected String user;
        protected String title;
        protected String rollbacktoken = null;
        protected int size = 0;
        protected int sizediff = 0;
        protected boolean summaryDeleted = false, userDeleted = false, contentDeleted = false;
        protected boolean pageDeleted = false;
        ...
        /**
         *  Sets sha1 for this revision.
         *  @param hash the sha1 value
         *  @since 0.24
         */
        ...
        /**
         *  Gets the sha1 value for the revision
         *  @return sha1 value
         */
     ...
     */
    public String encode(String text, boolean normalize) throws IOException
    {
